#!/usr/bin/perl 
# IronPort_DeliveryLog.pm - parse IronPort delivery log format
#
...
package IronPort_DeliveryLog;

use IO::File;
use IO::Handle '_IONBF';
use POSIX qw(strftime);
# use Python_Unpickler;

# python sneaks in a couple of null bytes after the string 
# field, for 4-byte alignment possibly.

my $RECORD_FORMAT = "a2NNN";
my $RECORD_SIZE   = 14;
my $MAX_VERSION_SUPPORTED = 4;
...
{
	my ($self, $UP, $fh) = @_;
	my ($raw, $kind, $data_len, $td_sec, $td_usec, $dtxt, $darr, $tm);
	my @tarr;
	$data_len = 0;

	$fh->read($raw, $RECORD_SIZE);
    ($kind, $data_len, $td_sec, $td_usec) = unpack($RECORD_FORMAT, $raw);
	if ($data_len > 0) { 
		$fh->read($dtxt, $data_len); 
		$darr = $UP->unpickle_string($dtxt);
	}

	
	if ($kind eq '<<')    { $self->_start($td_sec, $td_usec, $darr); }
	elsif ($kind eq '>>') { $self->_end($td_sec, $td_usec); }
	elsif ($kind eq 'HB') { $self->_bounce($td_sec, $td_usec, $darr->[0]); }
	elsif ($kind eq 'MD') { $self->_delv($td_sec, $td_usec, $darr->[0]); }
	else                  { $self->_error($kind, $fh); }
}
...
sub _start
{
	...
	# print "Start Type is ", $record->{Type}, "\n";
	$self->_append($record);
}
...
sub _bounce
{
	...
	if ($version == 0) { 
		$record = {
			...
			From     => $arr->[5],
			Code     => $arr->[6],
			Reason   => '',         # not in binary log
		};
		_build_errs($record, $arr->[7]);
		_build_rcpt($record, $arr->[8]);
		_build_cust($record, $arr->[10]);
	}
	elsif (($version >= 1) && ($version <= $MAX_VERSION_SUPPORTED)) { 
		$record = {
			Type     => $BOUNCE,
			Log_Time => [$sec, $usec],
			Time     => [$arr->[0], $arr->[1]],
		...
		_build_errs($record, $arr->[8]);
		_build_rcpt($record, $arr->[9]);
		_build_cust($record, $arr->[10]);
	...
	if ($version >= 4) {
	        $record->{SRC_IP} = $arr->[11];
	}
	else {
	        $record->{SRC_IP} = ''; # Not in this version
	}
...
sub _delv 
{
    ...
    # V1 ti_sec, ti_usec, bytes, mid, ip, env_from, domain, rcpt_data = data
    # customer_data = []
	if ($version == 0) {
		$record = {
			Type       => $DELIVERY,
		...
		_build_rcpt($record, $arr->[7]);
		_build_cust($record, $arr->[8]);
	}

	# V2 ti_sec, ti_usec, bytes, mid, ip, env_from, domain, rcpt_data, 
	# customer_data = data                
	elsif (($version >= 1) && ($version <= $MAX_VERSION_SUPPORTED)) {
		$record = {
			Type       => $DELIVERY,
...
sub _append
{
	...
	if ($type == $START    && (defined $self->{I_START}) ||
	    $type == $END      && (defined $self->{I_END}) ||
	    $type == $BOUNCE   && (defined $self->{I_BOUNCE}) ||

	    $type == $DELIVERY && (defined $self->{I_DELIVERY}) ||
	    $type == $ERROR    && (defined $self->{I_ERROR}))
	{ 1 }

	else {
		push @{$self->{APPEND}}, $record if defined ($self->{APPEND});
		&{$self->{CALLBACK}}($record) if defined ($self->{CALLBACK});
	}
...
sub _build_rcpt
{
	...
	my $rsz = $#result + 1;
	$rec->{Recipient_Information} = \@result;
}
...
sub _build_cust
{
	...
	$rec->{Customer_Information} = \@result;
}
...
sub Time_to_String
{
	...
	my $tm1 = strftime "%a %b %d %H:%M:%S.%%03d %Y", @arr;
	return sprintf $tm1, $usec / 1000;
}
...
sub ignore
{
	...
	foreach my $type (@types) {
		$self->{I_START}    = 1 if $type eq 'start';
		$self->{I_END}      = 1 if $type eq 'end';
		$self->{I_BOUNCE}   = 1 if $type eq 'bounce';
		$self->{I_DELIVERY} = 1 if $type eq 'delivery';
		$self->{I_ERROR}    = 1 if $type eq 'error';
	}
}
...
{
	my ($self, $name) = @_;
	my $unpickler = new Python_Unpickler();
	my $fh = new IO::File;

	if ($fh->open("< $name")) {
        binmode($fh);
		while (! $fh->eof) { 
			$self->_process_record($unpickler, $fh); 
		}
		$fh->close;
	}
